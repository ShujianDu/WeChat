...
function initSelect(obj, url, ajax_getName) {
	...
	if ($Obj_name.val() == "" || $Obj_name.val() == null) {
		...
		if (ajax_getName != null && ajax_getName != "") {
			$.get(ajax_getName,{ orgId: $Obj_name.val(), time: new Date().getTime() },  function(data) {
				$iDiv.text(data)
			});
		} else {
			$iDiv.text($Obj_name.val());
		}
	}
	$Obj.parent().append($iDiv);// 将DIV添加到text所在的父元素内
	// 鼠标事件
	// $iDiv.mouseover(function(){//鼠标移到
	// $iDiv.css("background","url(images/icon_select_focus.gif) no-repeat right
	// 4px");
	// });
	// $iDiv.mouseout(function(){//鼠标移走
	// $iDiv.css("background","url(images/icon_select.gif) no-repeat right
	// 4px");
	// });
	$iDiv.click(function() {
		if ($("#selectchild" + obj).length == 1) {
			...
			if (($("#selectchild" + obj + ":hidden").length == 1)) {
				...
				$("#selectchild" + obj).css("top",
						offset_div.top + offset_div.height + 2 + "px");
