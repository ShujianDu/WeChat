...
<header>
	...
	<div class="mg-tb-30">
		<c:if test="${cardApplyList.applyList eq null}">
			<div class="topOneB mar-1 allOneBradius">
				<table class="topTwo mb-10">
					<tr>
						<td><span class="">很抱歉！没有查询到您的申请进度。详情请咨询中国银行信用卡24小时客服热线4006695566。感谢您对中行信用卡的支持和厚爱！
						</span></td>
					</tr>
				</table>
			</div>
		...
		<c:if test="${cardApplyList.next eq true}">
			<form action="next.do" method="post" id="submitForm">
				<div style="margin-top: 10px;">
					<table class="topTwo">
						<tr>
							<td><input type="submit" class="HandInHui" value="下一页" /></td>
						</tr>
					</table>
				</div>
				<input type="hidden" id="name" name="name" value="${model.name}"> 
				<input type="hidden" id="idType" name="idType" value="${model.idType}">
