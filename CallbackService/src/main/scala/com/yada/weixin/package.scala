...
package object weixin {
  ...
  class WeixinCrypt(key: String, appId: String) {
    ...
    def encrypt(text: String) = {
      ...
      byteBuffer.flip()
      Base64.encodeBase64String(encryptCipher.doFinal(byteBuffer.array(), 0, byteBuffer.remaining()))
    }
  ...
  def xmlToJson(xmlData: NodeSeq): JsValue = {
    ...
    def mkFields(xs: List[(String, XElem)]) =
      xs.flatMap {
        case (name, value) => (value, toJValue(value)) match {
          ...
          case (_, json) => JsObject(Map(name -> json).toSeq) :: Nil
        }
      }
    ...
    def toJValue(x: XElem): JsValue = x match {
      ...
      case XLeaf((name, value), attributes) => (value, attributes) match {
        ...
        case (_, xs) => JsObject(Seq(JsObject(Map(name -> toJValue(value)).toSeq).toString() -> Json.toJson(mkFields(xs))))
      }
      case XNode(xs) => {
    ...
    buildNodes(xmlData) match {
      ...
      case List(x) => JsObject(Map(nameOf(xmlData.head) -> toJValue(x)).toSeq)
