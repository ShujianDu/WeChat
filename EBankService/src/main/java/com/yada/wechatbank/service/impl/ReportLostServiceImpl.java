...
/**
 * 信用卡挂失 Created by QinQiang on 2016/4/18.
 */
@Service
public class ReportLostServiceImpl extends BaseService implements ReportLostService {
	...
	public boolean creditCardReportLost(String cardNo, String identityType, String identityNo, String lossReason) {
		...
		param.put("idType", identityType);
		param.put("idNum", identityNo);
		param.put("familyName", name);
		param.put("lossReason", lossReason);
	...
	public String sendSMS(String identityType, String identityNo, String mobileNo) {
		...
		if (respMobileNo == null || !respMobileNo.equals(mobileNo)) {
			messageProducer.send(TopicEnum.EBANK_QUERY, "ReportLostSendSMS", "用户[" + identityNo + "]通过后台获取手机号[" + mobileNo + "]与后台返回的手机号匹配失败");
			return "wrongMobilNo";
		}
		// 发送短信验证码
